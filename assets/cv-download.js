(() => {
  ...
  async function buildPdfUrl() {
    ...
    window.__cvChunks = [];
    for (let number = 1; number <= chunkCount; number += 1) await loadChunk(number);
    if (window.__cvChunks.length !== chunkCount || window.__cvChunks.some((chunk) => !chunk)) {
      throw new Error("The CV data is incomplete.");
    }
    const binary = atob(window.__cvChunks.join(""));
    if (!binary.startsWith("%PDF-")) throw new Error("The reconstructed CV is not a valid PDF.");
  ...
  async function initialiseCv() {
    ...
    document.querySelectorAll("[data-cv-preview]").forEach((frame) => { frame.src = pdfUrl; });
  }
